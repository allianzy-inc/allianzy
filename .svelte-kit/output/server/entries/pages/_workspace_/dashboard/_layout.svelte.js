...
function _layout($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    var $$store_subs;
    let t, workspace, path, notifications, inboxNotifications, unreadCount, companies, currentUserRole, currentUserPermissions, canViewBilling, canViewSupport, menuItems, isSpanish;
    let data = $$props["data"];
    let clientRole = "";
    ...
    $$renderer2.push(`<!--]--> <aside${attr_class(`fixed inset-y-0 left-0 z-50 w-64 bg-background border-r flex flex-col transition-transform duration-200 ease-in-out lg:relative lg:translate-x-0 ${stringify(isMobileMenuOpen ? "translate-x-0" : "-translate-x-full")}`)}><div class="p-6 border-b h-16 flex items-center justify-between">`);
    if (workspace === "allianzy") {
      $$renderer2.push("<!--[-->");
      $$renderer2.push(`<a${attr("href", `/${stringify(workspace)}`)} class="block"><img${attr("src", logoLight)} alt="Allianzy" class="h-8 dark:hidden"/> <img${attr("src", logoDark)} alt="Allianzy" class="h-8 hidden dark:block"/></a>`);
    } else {
      $$renderer2.push("<!--[!-->");
      $$renderer2.push(`<h2 class="text-lg font-bold tracking-tight uppercase truncate">${escape_html(workspace)}</h2>`);
    }
    $$renderer2.push(`<!--]--> <button class="lg:hidden ml-auto">`);
    X($$renderer2, { class: "w-5 h-5" });
    $$renderer2.push(`<!----></button></div> `);
    if (companies.length > 0) {
      $$renderer2.push("<!--[-->");
      $$renderer2.push(`<div class="px-4 pb-2 pt-4"><div class="relative"><button class="w-full flex items-center justify-between p-2 rounded-md border bg-card hover:bg-accent transition-colors shadow-sm"${attr("disabled", companies.length <= 1, true)}><div class="flex items-center gap-2 overflow-hidden"><div class="w-6 h-6 rounded flex items-center justify-center shrink-0 bg-muted/50 border">`);
      ...
      $$renderer2.push(`<!--]--></div> <span class="text-sm font-medium truncate">${escape_html(selectedCompany?.name || "Empresa")}</span></div> `);
      if (companies.length > 1) {
        $$renderer2.push("<!--[-->");
        Chevrons_up_down($$renderer2, {
          class: "w-4 h-4 text-muted-foreground ml-2 shrink-0 opacity-50"
        });
      } else {
        $$renderer2.push("<!--[!-->");
      }
    ...
    $$renderer2.push(`<!--]--> <nav class="flex-1 p-4 space-y-1"><!--[-->`);
    ...
    $$renderer2.push(`<!--]--></nav> <div class="p-4 border-t space-y-4"><a href="mailto:support@allianzy.us" class="flex items-center gap-2 text-sm text-muted-foreground hover:text-primary transition-colors px-2">`);
    Circle_help($$renderer2, { class: "w-4 h-4" });
    $$renderer2.push(`<!----> ${escape_html(t.dashboard.footer.help_support)}</a> <div class="text-xs text-muted-foreground px-2 space-y-1"><p>© ${escape_html(t.dashboard.footer.rights)}</p> <p class="flex items-center gap-1">${escape_html(t.dashboard.footer.made_with_love)} `);
    Heart($$renderer2, { class: "w-3 h-3 text-red-500 fill-current" });
    $$renderer2.push(`<!----> ${escape_html(isSpanish ? "por Allianzy." : "by Allianzy.")}</p></div></div></aside> <main class="flex-1 flex flex-col overflow-hidden"><header class="h-16 bg-background border-b flex items-center justify-between px-4 md:px-8 shrink-0"><div class="flex items-center gap-4"><button class="lg:hidden">`);
