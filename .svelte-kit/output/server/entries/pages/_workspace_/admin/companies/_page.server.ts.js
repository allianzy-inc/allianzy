...
const load = async ({ params, url }) => {
  ...
  let selectedCompanyDetail = null;
  let detailCompanyUsers = [];
  let detailCompanyProjects = [];
  let allCompanies = [];
  try {
    ...
    if (!Number.isNaN(id)) {
      ...
      if (company) {
        ...
        try {
          ...
          const companyUserIds = userLinks.map((l) => l.userId).filter((uid) => uid != null);
  ...
  try {
    const countsByCompany = await db.select({
      companyId: userCompanies.companyId,
      count: sql`count(*)::int`
    }).from(userCompanies).groupBy(userCompanies.companyId);
    ...
    const ownerLinks = await db.select({
      companyId: userCompanies.companyId,
      userId: userCompanies.userId,
      email: users.email,
      firstName: users.firstName,
      lastName: users.lastName
    }).from(userCompanies).innerJoin(users, eq(userCompanies.userId, users.id)).where(eq(userCompanies.role, "owner"));
    const adminLinks = await db.select({
      companyId: userCompanies.companyId,
      userId: userCompanies.userId,
      email: users.email,
      firstName: users.firstName,
      lastName: users.lastName
    }).from(userCompanies).innerJoin(users, eq(userCompanies.userId, users.id)).where(eq(userCompanies.role, "admin"));
    const ownerByCompanyId = /* @__PURE__ */ new Map();
    ...
    try {
      ...
      if (ws.length > 0) {
        usersList = await db.select({ id: users.id, email: users.email, firstName: users.firstName, lastName: users.lastName }).from(users).where(eq(users.workspaceId, ws[0].id)).orderBy(users.email);
      } else {
        usersList = await db.select({ id: users.id, email: users.email, firstName: users.firstName, lastName: users.lastName }).from(users).orderBy(users.email);
      }
    } catch (_) {
      usersList = await db.select({ id: users.id, email: users.email, firstName: users.firstName, lastName: users.lastName }).from(users).orderBy(users.email);
    }
...
const actions = {
  createCompany: async ({ request }) => {
    ...
    const adminUserId = adminUserIdRaw ? Number(adminUserIdRaw) : void 0;
    try {
  ...
  updateCompany: async ({ request }) => {
    ...
    try {
      ...
      if (adminUserId !== void 0 && !Number.isNaN(adminUserId)) {
        ...
        const existing = await db.select({ id: userCompanies.id }).from(userCompanies).where(and(eq(userCompanies.companyId, id), eq(userCompanies.userId, adminUserId))).limit(1);
  ...
  updateCompanyDetail: async ({ request, params }) => {
    const formData = await request.formData();
  ...
  inviteCompanyUser: async ({ request, params }) => {
    const formData = await request.formData();
    const companyIdRaw = formData.get("company_id");
    const companyId = companyIdRaw ? Number(companyIdRaw) : NaN;
    ...
    if (existingUser) {
      ...
      const [newUser] = await db.insert(users).values({
        ...
        email,
        role: "staff",
        workspaceId: company.workspaceId ?? void 0
      }).returning();
