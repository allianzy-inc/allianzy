...
function _page($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    let password = "";
    let name = "";
    let isRegister = store_get($$store_subs ??= {}, "$page", page).url.searchParams.get("mode") === "register";
    let isLoading = false;
    store_get($$store_subs ??= {}, "$page", page).url.searchParams.get("redirectTo") || "";
    ...
    $$renderer2.push(`<!--]--> <h2 class="text-2xl font-bold text-foreground tracking-tight">`);
    if (isRegister) {
      $$renderer2.push("<!--[-->");
      $$renderer2.push(`${escape_html(lang === "es" ? "Crear cuenta" : "Create Account")}`);
    } else {
      $$renderer2.push("<!--[!-->");
      $$renderer2.push(`${escape_html(lang === "es" ? "Bienvenido de nuevo" : "Welcome Back")}`);
    }
    $$renderer2.push(`<!--]--></h2> <p class="text-sm text-muted-foreground mt-2">`);
    if (isRegister) {
      $$renderer2.push("<!--[-->");
      $$renderer2.push(`${escape_html(lang === "es" ? "Ingresa tus datos para comenzar." : "Enter your details to get started.")}`);
    } else {
      $$renderer2.push("<!--[!-->");
      $$renderer2.push(`${escape_html(lang === "es" ? "Ingresa tus credenciales para acceder a tu cuenta." : "Enter your credentials to access your account.")}`);
    }
    ...
    if (isRegister) {
      $$renderer2.push("<!--[-->");
      $$renderer2.push(`<div class="space-y-2"><label class="text-xs font-medium uppercase tracking-wider text-muted-foreground ml-1">${escape_html(lang === "es" ? "Nombre completo" : "Full Name")}</label> <div class="relative group">`);
      User($$renderer2, {
        class: "absolute left-3 top-2.5 h-5 w-5 text-muted-foreground group-focus-within:text-primary transition-colors"
      });
      $$renderer2.push(`<!----> <input type="text"${attr("value", name)} required class="flex h-11 w-full rounded-xl border border-input bg-white/50 dark:bg-black/20 px-10 py-2 text-sm ring-offset-background placeholder:text-muted-foreground/70 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/50 focus-visible:border-primary transition-all shadow-sm"${attr("placeholder", lang === "es" ? "Juan Pérez" : "John Doe")}/></div></div>`);
    } else {
      $$renderer2.push("<!--[!-->");
    }
    $$renderer2.push(`<!--]--> <div class="space-y-2"><label class="text-xs font-medium uppercase tracking-wider text-muted-foreground ml-1">${escape_html(lang === "es" ? "Correo electrónico" : "Email Address")}</label> <div class="relative group">`);
    Mail($$renderer2, {
      class: "absolute left-3 top-2.5 h-5 w-5 text-muted-foreground group-focus-within:text-primary transition-colors"
    });
    $$renderer2.push(`<!----> <input type="email"${attr("value", email)} required class="flex h-11 w-full rounded-xl border border-input bg-white/50 dark:bg-black/20 px-10 py-2 text-sm ring-offset-background placeholder:text-muted-foreground/70 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/50 focus-visible:border-primary transition-all shadow-sm" placeholder="name@example.com"/></div></div> <div class="space-y-2"><div class="flex items-center justify-between ml-1"><label class="text-xs font-medium uppercase tracking-wider text-muted-foreground">${escape_html(lang === "es" ? "Contraseña" : "Password")}</label></div> <div class="relative group">`);
    Lock($$renderer2, {
      class: "absolute left-3 top-2.5 h-5 w-5 text-muted-foreground group-focus-within:text-primary transition-colors"
    });
    $$renderer2.push(`<!----> <input${attr("type", "password")}${attr("value", password)} required class="flex h-11 w-full rounded-xl border border-input bg-white/50 dark:bg-black/20 px-10 py-2 text-sm ring-offset-background placeholder:text-muted-foreground/70 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/50 focus-visible:border-primary transition-all shadow-sm pr-10" placeholder="••••••••"/> <button type="button" class="absolute right-3 top-3 text-muted-foreground hover:text-foreground transition-colors">`);
    {
      $$renderer2.push("<!--[!-->");
    ...
    if (!isRegister) {
      $$renderer2.push("<!--[-->");
      $$renderer2.push(`<div class="flex justify-end mt-1"><a${attr("href", forgotPasswordUrl)} class="text-xs font-medium text-primary hover:text-primary/80 transition-colors">${escape_html(lang === "es" ? "¿Olvidaste tu contraseña?" : "Forgot password?")}</a></div>`);
    } else {
      $$renderer2.push("<!--[!-->");
    }
    $$renderer2.push(`<!--]--></div> <button type="submit"${attr("disabled", isLoading, true)} class="w-full h-11 mt-4 bg-primary text-primary-foreground rounded-xl font-semibold hover:opacity-90 transition-all active:scale-[0.98] shadow-lg shadow-primary/20 disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center gap-2">`);
    {
      $$renderer2.push("<!--[!-->");
      if (isRegister) {
        $$renderer2.push("<!--[-->");
        ...
        $$renderer2.push("<!--[!-->");
        $$renderer2.push(`${escape_html(lang === "es" ? "Iniciar sesión" : "Sign In")}`);
      }
      $$renderer2.push(`<!--]--> <span class="text-lg">→</span>`);
    }
    $$renderer2.push(`<!--]--></button></form> <div class="mt-8 pt-6 border-t border-border/50"><div class="text-center text-sm"><span class="text-muted-foreground">`);
    if (isRegister) {
      $$renderer2.push("<!--[-->");
      $$renderer2.push(`${escape_html(lang === "es" ? "¿Ya tienes una cuenta?" : "Already have an account?")}`);
    } else {
      $$renderer2.push("<!--[!-->");
      $$renderer2.push(`${escape_html(lang === "es" ? "¿No tienes una cuenta?" : "Don't have an account?")}`);
    }
    $$renderer2.push(`<!--]--></span> <button class="ml-1 font-semibold text-primary hover:underline focus:outline-none transition-colors">`);
    if (isRegister) {
      $$renderer2.push("<!--[-->");
      $$renderer2.push(`${escape_html(lang === "es" ? "Iniciar sesión" : "Sign in")}`);
    } else {
      $$renderer2.push("<!--[!-->");
      $$renderer2.push(`${escape_html(lang === "es" ? "Registrarse" : "Sign up")}`);
    }
